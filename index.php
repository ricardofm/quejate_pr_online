<!DOCTYPE html>
<html lang="es">
	<head>
		<title>Quéjate PR Online</title>
		<meta charset="utf-8">
		<link type="text/css" rel="stylesheet" href="module1.css"/>
		<link type="image/png" rel="icon" href="pictures/icon.png"/>
		<meta name="viewport" content="width=device-width, initial-scale=1.0">
	</head>
	<body>
		<header>
			<div id="header">
					<h1>¡Bienvenidos a <i>Quéjate <span id="puerto">P</span><span 
					id="rico">R</span> Online</i>!</h1>
					<h2>Un sitio en donde puedes hacer saber tus reclamos...</h2>
			</div>
		</header>
	
		<nav>
			<div id="nav">
				<ul>
					<li class="links">Principal</li>
					<li><a class="links" href="foro.php">Foro</a></li>
					<li><a class="links" href="atu_municipio.php">A tu Municipio</a></li>
					<li><a class="links" href="contacto.php">Contacto</a></li>
					<li><a class="links" href="ayuda.php">Ayuda</a></li>
				</ul>
			</div>
			<div id="nav2">
				<div id="login"><a href="crear_cuenta.php">Crear cuenta</a></div>
			</div>
			<div id="nav3">
				<div id="signin"><a href="inicio_sesion.php">Iniciar cuenta</a></div>
			</div>
		</nav>
		<aside>
			<div class="flags">
							<img id="pr_flag" src="pictures/pr_flag.png" alt="PR_Flag"/>
							<img id="us_flag" src="pictures/us_flag.png" alt="PR_Flag"/>
			</div>
		</aside>
		<article>
			<div id="goal">
				<p>Esta página es para las personas que quieren:</p>
				<ul>
					<li>Acción por parte del Gobierno.</li>
					<li>Dar a conocer sus inquietudes.</li>
					<li>Bienestar para los suyos.</li>
					<li>Comunicarse sanamente.</li>
					<li>Escribir los hechos.</li>
				</ul>
				<p>No toleramos las personas que quieren:</p>
				<ul>
					<li>Decir palabras ofensivas.</li>
					<li>Amenazar.</li>
					<li>Burlarse.</li>
					<li>Mentir.</li>
				</ul>
			</div>
		</article>
		<footer>
			<div id="footer"><i>Copyright &copy; 2017-2018. Todos los derechos reservados.</i></div>
		</footer>
	</body>
</html>